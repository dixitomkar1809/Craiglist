...
// get specific user
app.get('/api/user/:useremail/:password', function (request, result) {
    var collection = db.get('users');
    collection.find({
...
app.get('/api/service/:searchInput', function (request, result) {
    var collection = db.get('Service');
    var value = request.params.searchInput;
    var pattern = "^value"
    // console.log(value);
    collection.find({ "serviceName": value }, function (err, services) {
        if (err) throw err;
        result.json(services);
    });
});
...
app.get('/api/serviceCategory/:servicecategory', function (request, result) {
    var collection = db.get('Service');
    collection.find({
        serviceCategory: request.params.servicecategory
